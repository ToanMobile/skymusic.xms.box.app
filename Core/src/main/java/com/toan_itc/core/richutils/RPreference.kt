...
class RPreference private constructor(context: Context) {
     ...
     * @return Char object
     */
    @JvmOverloads fun getChar(key: String, def: Char = '\u0000'): Char? = getValue({ pref.getString(key, def.toString())
        ?.get(0) }, def)
     ...
     */
    @JvmOverloads fun getDouble(key: String, def: Double = 0.toDouble()): Double = getValue({ pref.getString(key, def.toString())?.toDouble()?:0.0 }, def)
